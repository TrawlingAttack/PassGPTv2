...
class PcfgGrammar:
    ...
    def _recursive_guesses(self, cur_guess, pt, limit=None):
        ...
        # If it is a capitalization mask
            ...
            for mask in self.grammar[pt_type][index]['values']:
                ...
                if len(pt) == 1:
                    num_guesses += 1
                    self.print_guess(self.list_prefix,new_guess)
    ...
    def _honeyword_recursive_guess(self, cur_guess, pt, limit = None):
        ...
        # If it is a capitalization mask
            ...
            if len(pt) == 1:
                ...
                num_recursive_guesses = self._honeyword_recursive_guess(new_guess, pt[1:], limit)
                num_guesses += num_recursive_guesses
    ...
    def omen_generate_guesses(self, markov_cracker, limit=None):
        ...
        while guess is not None:
            ...
            # Output the results
            self.print_guess(self.list_prefix,guess)
            # Check the limit
            if limit:
                limit = limit - 1
                if limit <= 0:
    ...
    def print_guess(self,list_prefix, guess):
        """
        ...
        if not self.debug:
            try:
                if len(self.list_prefix) > 0:
                    self.list_password.append(random.choice(list_prefix) + guess)
                else:
                    self.list_password.append(guess)
                # if len(self.list_prefix) > 0:
                #     print(random.choice(list_prefix) + guess)
                # else:
                #     print(guess)
