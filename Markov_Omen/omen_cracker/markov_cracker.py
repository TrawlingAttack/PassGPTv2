...
class MarkovCracker:
    ...
    ############################################################################################
    def __init__(self, grammar, version, base_directory, session_name, rule_name, uuid,prefix, optimizer = None, restore = False):
    ...
    def next_guess(self, level = None):
        ...
        if self.cur_guess == None:
            ...
            ##--Tìm trạng thái Markov tương ứng với prefix
            ##--Tìm trạng thái Markov tương ứng với prefix
            if self.prefix and isinstance(self.grammar['ip'], list):
                for i, seq in enumerate(self.grammar['ip']):
                    if isinstance(seq, list):  # Kiểm tra xem seq có thể lặp qua không
                        for j, entry in enumerate(seq):
                            if isinstance(entry, str) and entry.startswith(self.prefix):
                                self.cur_ip = [i, j]
                                break
                        else:
                            continue
